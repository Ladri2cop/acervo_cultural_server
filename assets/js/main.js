$(document).ready(function () {
  ...
  $("body").on("click", ".confirmar", async function (e) {
    ...
    const mensaje =
      $(this).data("mensaje") ?? "No se puede revertir esta acción.";
  ...
  function init_tooltips() {
    if (
      [
        ...
        "bs_vapor",
        "bs_zephyr",
      ].includes(Bee.css_framework) != true
    )
      return;
  ...
  $("body").on(
    "click",
    ".delete-bulma-notification",
    delete_bulma_notification
  );
  ...
  function init_toastr_setup() {
    ...
    // Detecta si estás dentro de /admin
    ...
    toastr.options = {
      ...
      newestOnTop: true,
      progressBar: true,
      positionClass: isAdmin ? "toast-bottom-right" : "toast-top-right",
...
function init_db_test() {
  ...
  alert.innerHTML =
    '<i class="fas fa-spinner fa-spin"></i> Probando conexión a la base de datos...';
...
async function test_api() {
  ...
  if (res.status === 200) {
    toastr.success(
      `API funcional, fueron cargados <b>${res.data.length}</b> registros.`,
      "Prueba de la API"
    );
  } else {
